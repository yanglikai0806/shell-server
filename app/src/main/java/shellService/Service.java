...
public class Service {
    ...
    public Service(ServiceGetText serviceGetText, int PORT) {
        ...
        try {
            /** 创建ServerSocket*/
            // 创建一个ServerSocket在端口3721监听客户请求
            ServerSocket serverSocket = new ServerSocket(PORT);
//            LogUtil.d("","服务端运行在" + PORT + "端口");
            while (true) {
                // 侦听并接受到此Socket的连接,请求到来则产生一个Socket对象，并继续执行
                Socket socket = serverSocket.accept();
//                LogUtil.d("","监听请求到来则产生一个Socket对象，并继续执行");
                new CreateServerThread(socket);//当有请求时，启一个线程处理
            }
        } catch (Exception e) {
            LogUtil.d("","shell server连接监听发生错误 Exception:" + e);
        }finally{
//            serverSocket.close();
        }
    }
...
//                System.out.println("while循环：客户端返回 : " + line);
                LogUtil.d("","服务器已接收：" + line);
                String repeat = mServiceGetText.getText(line + "");
                LogUtil.d("","服务器将返回：" + repeat);
...
//                System.out.println("while循环：已刷新返回");
                ...
                LogUtil.d("","socket 连接线程发生错误：" + e.toString());
            }
        }
    }
