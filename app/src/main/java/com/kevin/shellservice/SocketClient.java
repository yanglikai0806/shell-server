package com.kevin.shellservice;
...
//            LogUtil.d(TAG, "与shell service进行socket通讯,超时为：" + 3000);
            /** 发送客户端准备传输的信息 */
            ...
            e.printStackTrace();
            LogUtil.d(TAG, "与shell service进行socket通讯发生错误" + e);
...
//                        if (line != null)mOnServiceSend.getSend(line);
                    ...
                    LogUtil.d(TAG, "客户端接收解析服务器的while循环结束");
                } catch (Exception e){
                    e.printStackTrace();
                    LogUtil.d(TAG, "客户端接收解析服务器的Threadcatch块执行：" + e.toString());
                }
